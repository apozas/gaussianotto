...
for cyc=1:Ncycle
    ...
    for i=1:ntH
        ...
        % Total mutual information between detector and baths
        MutInfTotal(cyc,i) = Entropy(sigDetH)+Entropy(sigBathsH) ...
                            -Entropy(sigH);
        % Mutual information between detector and individual bath modes 
        for j=1:NC+NH
		    sigMode = sigH(2*j+1:2*j+2,2*j+1:2*j+2);
            sigComb = [sigDetH(1:2,1:2),sigH(1:2,2*j+1:2*j+2);sigH(2*j+1:2*j+2,1:2),sigMode];
...
% Detector-bath components MI (Figure 5)
